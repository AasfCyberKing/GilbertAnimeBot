...
#sleep how many times after each edit in 'stbxforce' 
EDIT_SLEEP = 1
#edit how many times in 'stbxforce' 
EDIT_TIMES = 4



STB_IMG ="https://telegra.ph/file/722ede42967fe0d027748.mp4"

stb_gay = [
         "🤔WHO IS NO 1 GAY",
         "🤔WHO IS NO 1 NOOB",
         " 🤔WHO IS XFORCE GUY",
         " 🤔WHO IS VERY DAY WATCHING NSFW",
]
...
@run_async
def stbxforce(update: Update, context: CallbackContext):
    bot, args = context.bot, context.args
    msg = update.effective_message.reply_text('searching on google') 
    for x in range(EDIT_TIMES):
        msg.edit_text(stb_gay[x%4])
        time.sleep(EDIT_SLEEP)
    update.effective_message.reply_video(
            STB_IMG, caption=f"the no 1 Cringe of king XForceSTB Gay  Danceing Now!!!")
...
STBXFORCE_HANDLER = DisableAbleCommandHandler("stbxforce",stbxforce)
...
dispatcher.add_handler(STBXFORCE_HANDLER)
